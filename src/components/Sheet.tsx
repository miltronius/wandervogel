...
function Chip({
  ...
  const base =
    "whitespace-nowrap rounded-full border px-[11px] py-[5px] text-[11.5px] font-medium transition-colors duration-150";
  const off =
    "border-line bg-transparent text-muted hover:border-muted-2 hover:text-text";
  const onCls = meadow
    ? "border-meadow bg-meadow text-[#0c1503] font-semibold"
    : "border-marker bg-marker text-[#1a1500] font-semibold";
  return (
    ...
    >
      ...
      <div className="relative z-[1] border-b border-line p-[18px_20px_14px] max-md:pt-1.5">
        ...
        <p className="max-w-[31ch] text-[12.5px] leading-[1.45] text-muted">
          Schöne Touren abseits der viralen Hotspots — mit Ruhe-Index, markanten
          Aussichten und allem für die Anreise.
        </p>
      ...
      {activeTab === "plan" ? (
        ...
        <>
          <div className="relative z-[1] border-b border-line p-[14px_20px]">
            <FilterRow label="Ruhe — wie einsam darf's sein?">
              <Chip
                ...
                on={filters.ruhe === "all"}
                onClick={() =>
                  onFiltersChange({ ...filters, ruhe: "all" as RuheFilter })
                }
              >
                Alle
              </Chip>
              <Chip
                meadow
                on={filters.ruhe === "quiet"}
                onClick={() =>
                  onFiltersChange({ ...filters, ruhe: "quiet" as RuheFilter })
                }
              >
                Nur ruhig & einsam
              </Chip>
            ...
            <FilterRow label="Schwierigkeit (SAC-Skala)">
              <Chip
                on={filters.diff === "all"}
                onClick={() =>
                  onFiltersChange({ ...filters, diff: "all" as DiffFilter })
                }
              >
                Alle
              </Chip>
              <Chip
                on={filters.diff === "T1"}
                onClick={() =>
                  onFiltersChange({ ...filters, diff: "T1" as DiffFilter })
                }
              >
                T1–T2 leicht
              </Chip>
              <Chip
                on={filters.diff === "T3"}
                onClick={() =>
                  onFiltersChange({ ...filters, diff: "T3" as DiffFilter })
                }
              >
                T3 anspruchsvoll
              </Chip>
              <Chip
                on={filters.diff === "T4"}
                onClick={() =>
                  onFiltersChange({ ...filters, diff: "T4" as DiffFilter })
                }
              >
                T4 alpin
              </Chip>
            ...
            <FilterRow label="Dauer" last>
              <Chip
                on={filters.dur === "all"}
                onClick={() =>
                  onFiltersChange({ ...filters, dur: "all" as DurFilter })
                }
              >
                Alle
              </Chip>
              <Chip
                on={filters.dur === "half"}
                onClick={() =>
                  onFiltersChange({ ...filters, dur: "half" as DurFilter })
                }
              >
                Halbtag ≤ 4 h
              </Chip>
              <Chip
                on={filters.dur === "full"}
                onClick={() =>
                  onFiltersChange({ ...filters, dur: "full" as DurFilter })
                }
              >
                Tagestour
              </Chip>
          ...
          <div className="relative z-[1] border-t border-line p-[10px_20px_16px] text-[10.5px] leading-[1.5] text-muted-2">
            <b className="text-muted">Karten:</b> © swisstopo (Landeskarte,
            Relief, Luftbild) · <b className="text-muted">Wege:</b> offizielles
            Wanderwegnetz swisstopo &amp; SchweizMobil/ASTRA — live als Layer
            einblendbar. Dauer, Höhenmeter und Ruhe-Index sind kuratierte
  ...
  return (
    <button
      className={`flex-1 p-[11px_0] text-center font-mono text-[11px] uppercase tracking-[.12em] transition-colors duration-150 ${
        active
          ? "border-b-2 border-marker text-marker"
...
function FilterRow({
  ...
  return (
    <div className={`flex flex-wrap gap-[6px] ${last ? "" : "mb-[11px]"}`}>
      <div className="mb-0.5 w-full font-mono text-[9.5px] uppercase tracking-[.18em] text-muted-2">
        {label}
      </div>
      {children}
    </div>
